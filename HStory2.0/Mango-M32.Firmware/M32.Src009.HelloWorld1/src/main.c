...
FlagStatus USART_GetFlagStatus(USART_TypeDef* USARTx, uint16_t USART_FLAG)
{
  ...
  if ((USARTx->SR & USART_FLAG) != (uint16_t)RESET)   //USART_FLAG_TXE를 받았을 떄 1이면 1로 
  {                                                   
    bitstatus = SET;
  }
  else                                               // 0이면 0으로 
...
{
    USART_InitTypeDef USART_InitStructure;      ////구조문의 이름. 
...
void GPIO_Configuration(void)
{
    ...
    GPIO_Init(GPIO_USART, &GPIO_InitStructure);   //////////////모르는 부분 ////////////////////
