...
void DMA_Configuration(void)
{
	...
	DMA_Init(DMA1_Channel2, &DMA_InitStructure);//DMA_Init(DMA1_Channel2, &DMA_InitStructure);
	DMA_Init(DMA1_Channel3, &DMA_InitStructure);//DMA_Init(DMA1_Channel3, &DMA_InitStructure);
...
void ADC_Configuration(void)
{
    ...
    ADC_InitStructure.ADC_ContinuousConvMode = ENABLE;//ENABLE;	//if disable -> only once sensing
...
{ 
	...
	/*---------------Period of Control-----------------*/
	TIM_TimeBaseStructure.TIM_Period = 299; ;//499;//29; 		// 0.0001s 0.002         
	...
	TIM_TimeBaseStructure.TIM_Period = 999; 		// 1us*100= 100usec         
    TIM_TimeBaseStructure.TIM_Prescaler = 71;//rcc_clocks.PCLK1_Frequency*2/1000000;       
...
{ 
    ...
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;	//priority sub in each group xxxxx
